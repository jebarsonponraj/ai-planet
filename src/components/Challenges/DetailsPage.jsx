...
const DetailsPage = () => {
  ...
  const handleDelete = () => {
    ...
    if (storedHackathons) {
      const hackathons = JSON.parse(storedHackathons);
      const updatedHackathons = hackathons.filter(h => h.id.toString() !== id);
  ...
  if (!challengeData) {
    return <div className="bg-white min-h-screen text-black p-8">Loading...</div>;
  }
    ...
    <div className="bg-white min-h-screen text-white">
      <div className="bg-[#003145] p-8">
        <div className="inline-flex items-center bg-[#FFCE5C] text-black px-3 py-1 rounded-md mb-4">
          ...
          <span className="text-sm">Starts on {new Date(challengeData.startDate).toLocaleDateString()}</span>
        </div>
      ...
      <div className="bg-white text-black">
        <div className="shadow-lg rounded-lg p-6 mb-4 flex justify-between items-start">
          ...
          <div className="space-x-2">
            <button onClick={handleEdit} className="bg-[#44924C] text-white px-4 py-2 rounded">Edit</button>
            <button onClick={() => setIsModalOpen(true)} className="border border-[#DC1414] text-[#DC1414] px-4 py-2 rounded">Delete</button>
          </div>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white text-black p-6 rounded-lg shadow-lg">
            ...
            <p className="mb-4">Are you sure you want to delete this challenge?</p>
            <div className="flex justify-end space-x-2">
              <button onClick={() => setIsModalOpen(false)} className="bg-gray-300 text-black px-4 py-2 rounded">Cancel</button>
              <button onClick={handleDelete} className="bg-[#DC1414] text-white px-4 py-2 rounded">Delete</button>
            </div>
          </div>
        </div>
      )}
