...
const HackathonCard = ({ hackathon }) => {
    ...
    if (now <= end) {
      const calculateTimeLeft = () => {
        ...
        if (timeDifference > 0) {
          ...
          setRemainingTime(`${days} : ${hours.toString().padStart(2, '0')} : ${minutes.toString().padStart(2, '0')}`);
        } else {
  ...
  const formatTimeLeft = (timeString) => {
    const [days, hours, minutes] = timeString.split(':').map(part => part.trim());
    return (
  ...
  return (
    <div className="flex flex-col bg-white rounded-[15px] shadow-md overflow-hidden w-full max-w-xs mx-auto h-[400px]">
      ...
      <div className="p-4 text-center w-full flex flex-col justify-between flex-grow">
        <div>
          <p className={`text-xs font-medium mb-1 px-2 py-1 rounded-md inline-block ${
              status === 'Upcoming' ? 'bg-[#FCF1D2]' :
              status === 'Active' ? 'bg-[#D2E5D4]' :
              'bg-[#FFDED4]'
          }`}>
            {status}
          </p>
          ...
          <p className="text-xs text-gray-600 mb-1">
            {status === 'Past' ? 'Ended on' : status === 'Active' ? 'Ends in' : 'Starts in'}
          </p>
          {status === 'Past' ? (
            <p className="text-md font-bold">{new Date(endDate).toLocaleDateString()}</p>
          ) : (
